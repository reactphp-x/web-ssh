...
5. 终端页右侧 **AI 助手**（需配置 `NEURON_AI_KEY`）可描述运维任务；顶部可切换左侧 **实时现场 / 终端**（终端可手动输入）
...
SSH 连接成功后，终端页右侧出现 **AI 助手** 侧栏（移动端为「终端 | AI 助手」标签切换）。从 **主机管理** 点击 **AI 助手** 进入**同一终端页**，默认左侧为 **实时现场**，右侧为 AI 对话；可在 AI 侧栏顶部切换左侧为 **交互终端** 并手动输入。
...
| 终端页 AI 侧栏顶部切换 | 左侧在 **实时现场**（旁路输出）与 **交互终端**（可手动输入）间切换 |
| 菜单 **实时现场** | 多连接监控墙 |
