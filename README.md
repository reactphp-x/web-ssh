...
4. 进入 **终端**，选择主机建立 SSH 会话
5. 右侧 **AI 助手**（需配置 `NEURON_AI_KEY`）可描述运维任务；底部 **批准/拒绝** 后才会执行命令
...
SSH 连接成功后，终端页右侧出现 **AI 助手** 侧栏（移动端为「终端 | AI 助手」标签切换）。
...
| 终端页 AI 侧栏 **实时现场** | 只看当前标签页连接，默认折叠，点击展开 |
